...
28-09-2019 Slightly modified by L. Forattini to fix timing issues
...
void loop() { 
  int k=1;
  int n;
  interrupts(); Serial.print("1st Pass writing 0000:"); Serial.flush(); noInterrupts(); fill(0);
  interrupts(); Serial.print("2nd Pass writing 1111:"); Serial.flush(); noInterrupts(); fill(15);
  interrupts(); Serial.print("3rd Pass writing 1010:"); Serial.flush(); noInterrupts(); fill(10);
  interrupts(); Serial.print("4rth Pass writing 0101:"); Serial.flush(); noInterrupts(); fill(5);
  if (++k>loops) {
    interrupts(); Serial.print("Test Completed. "); Serial.flush(); 
    if (err){
      Serial.print("Test Ended with errors!");
...
void fill( int v) {
  ...
  //  for (c=0; c < 2; c++) {
      ...
      //sprintf(text, " Row|Column|Value|ReadValue %d|%d|%d|%d ", r, c, v, Valuta);
      //interrupts(); Serial.println(text); Serial.flush(); noInterrupts();
      if (Valuta != v) {
        adr = (c << ADDR_BUS_SIZE) + r;
        interrupts(); Serial.print("Error @address= ");Serial.println(adr); noInterrupts();
        err=true;  
      }
      //if ((Valuta != v) | dbg) Serial.println(" "); 
    }
    blink();   // Blink LED Row every iteration
  }
  if (!err) {
      interrupts();
      Serial.println("OK");
...
//7 WE low WRITE ENABLE
  delay(dellay);         // need a little delay here? TCAH
  digitalWrite(WE,LOW);
...
/*   
  ...
  //3 WE high DISABLE WRITE ENABLE
  digitalWrite(WE,HIGH);
